...
const page = () => {
    ...
    const onDropRejected = (rejectedFiles: FileRejection[]) => {
        ...
        toast({
            ...
            description: "Please choose a PNG, JPG, or JPEG image type instead",
            variant: "destructive",
        })
    }
                ...
                >
                    {({ getRootProps, getInputProps }) => (
                        <div className='h-full w-full flex-1 flex flex-col items-center justify-center' {...getRootProps()}>
                            <input {...getInputProps()} />
                            {isDragOver ? <MousePointerSquareDashed className="h-6 w-f text-zinc-500 mb-2" /> : isUploading || isPending ? (<Loader2 className="animate-spin h-6 w-6 text-zinc-500 mb-2" />) : (<Image className="h-6 w-6 text-zinc-500 mb-2" />)}
